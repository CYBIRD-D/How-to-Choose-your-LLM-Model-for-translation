...
更大的模型/更快的速度=更好的效果=更多的显存+更多的cuda core=更贵的api价格
  ...
  - 谷歌提供免费层api，且gemini 2.0flash通常强于70%开源小模型，但有额度限制 更大的模型=更多的限制</br>
    - 如果没有8G及以上显卡（最低条件6G）（Mac为16G统一内存），建议选择google free api
...
> 但会受请求/分钟 (RPM)” “Token/分钟 (TPM)” “请求/天 (RPD)”等速率限制影响。每天配额按太平洋时间午夜重置
...
\*AI Studio vs API：AI Studio 的使用永久免费，但其界面内的限额与 API 文档表格不总是完全一致；Google 会不定期调整（例如 2.5 Flash 的 RPD 曾被观察到从 500 降到 250）
