...
## LLM的优势？
LLM（大语言模型）相较传统神经机器翻译（NMT，通常指专门为翻译训练的序列到序列/Transformer 系统）并非在所有指标上全面碾压，但在若干关键方面具有明显优势，尤其是上下文，术语表与跨语言能力等
